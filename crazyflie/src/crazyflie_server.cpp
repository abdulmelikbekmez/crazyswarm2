...
{
  public:
    CrazyflieLogger(rclcpp::Logger logger) : Logger(), logger_(logger)
    {
    }
...
class CrazyflieROS
{
  ...
  public:
    CrazyflieROS(const std::string &link_uri, const std::string &cf_type, const std::string &name, rclcpp::Node *node,
        ...
        : logger_(rclcpp::get_logger(name)), cf_logger_(logger_),
          cf_(link_uri, cf_logger_, std::bind(&CrazyflieROS::on_console, this, std::placeholders::_1)), name_(name),
          node_(node), tf_broadcaster_(node)
    {
        ...
        service_start_trajectory_ = node->create_service<StartTrajectory>(
            name + "/start_trajectory", std::bind(&CrazyflieROS::start_trajectory, this, _1, _2));
        service_takeoff_ =
            node->create_service<Takeoff>(name + "/takeoff", std::bind(&CrazyflieROS::takeoff, this, _1, _2));
        ...
        service_upload_trajectory_ = node->create_service<UploadTrajectory>(
            name + "/upload_trajectory", std::bind(&CrazyflieROS::upload_trajectory, this, _1, _2));
        ...
        subscription_cmd_vel = node->create_subscription<geometry_msgs::msg::Twist>(
            name + "/cmd_vel", rclcpp::SystemDefaultsQoS(), std::bind(&CrazyflieROS::cmd_vel_changed, this, _1));
            ...
            if (logging_enabled)
            {
                ...
                for (const auto &i : log_config_map)
                {
                    ...
                    if (i.first.find("default_topics.pose") == 0)
                    {
                        ...
                        std::function<void(uint32_t, const logPose *)> cb = std::bind(
                            &CrazyflieROS::on_logging_pose, this, std::placeholders::_1, std::placeholders::_2);
                    ...
                    else if (i.first.find("default_topics.scan") == 0)
                    {
                        ...
                        std::function<void(uint32_t, const logScan *)> cb = std::bind(
                            &CrazyflieROS::on_logging_scan, this, std::placeholders::_1, std::placeholders::_2);
                    ...
                    else if (i.first.find("custom_topics") == 0 && i.first.rfind(".vars") != std::string::npos)
                    {
                        ...
                        auto vars =
                            log_config_map["custom_topics." + topic_name + ".vars"].get<std::vector<std::string>>();

                        RCLCPP_INFO(logger_, "Logging to %s at %d Hz", topic_name.c_str(), freq);

                        publishers_generic_.emplace_back(
                            node->create_publisher<crazyflie_interfaces::msg::LogDataGeneric>(name + "/" + topic_name,
                                                                                              10));

                        std::function<void(uint32_t, const std::vector<float> *, void *userData)> cb =
                            std::bind(&CrazyflieROS::on_logging_custom, this, std::placeholders::_1,
    ...
    void change_parameter(const rclcpp::Parameter &p)
    {
        ...
        {
            RCLCPP_ERROR(logger_, "Incorrect parameter update request for param \"%s\"", p.get_name().c_str());
            return;
        }
        ...
        RCLCPP_INFO(logger_, "Update parameter \"%s.%s\" to %s", group.c_str(), name.c_str(),
  ...
  private:
    ...
    {
        RCLCPP_INFO(logger_,
                    "start_trajectory(id=%d, timescale=%f, reversed=%d, "
                    "relative=%d, group_mask=%d)",
                    request->trajectory_id, request->timescale, request->reversed, request->relative,
                    request->group_mask);
    ...
    {
        RCLCPP_INFO(logger_, "takeoff(height=%f m, duration=%f s, group_mask=%d)", request->height,
    ...
    {
        RCLCPP_INFO(logger_, "land(height=%f m, duration=%f s, group_mask=%d)", request->height,
                    rclcpp::Duration(request->duration).seconds(), request->group_mask);
    ...
    {
        RCLCPP_INFO(logger_,
                    "go_to(position=%f,%f,%f m, yaw=%f rad, duration=%f s, "
                    "relative=%d, group_mask=%d)",
    ...
    {
        RCLCPP_INFO(logger_, "upload_trajectory(id=%d, offset=%d)", request->trajectory_id, request->piece_offset);
    ...
    void on_logging_custom(uint32_t time_in_ms, const std::vector<float> *values, void *userData)
    {
        ...
        auto pub =
            reinterpret_cast<rclcpp::Publisher<crazyflie_interfaces::msg::LogDataGeneric>::SharedPtr *>(userData);
...
{
  public:
    CrazyflieServer() : Node("crazyflie_server"), logger_(rclcpp::get_logger("all"))
    {
        // topics for "all"
        ...
        service_start_trajectory_ = this->create_service<StartTrajectory>(
            "all/start_trajectory", std::bind(&CrazyflieServer::start_trajectory, this, _1, _2));
        service_takeoff_ =
            this->create_service<Takeoff>("all/takeoff", std::bind(&CrazyflieServer::takeoff, this, _1, _2));
  ...
  private:
                          ...
                          std::shared_ptr<StartTrajectory::Response> response)
    {
        ...
        for (int i = 0; i < broadcasts_num_repeats_; ++i)
        {
            ...
            {
                auto &cfbc = bc.second;
                cfbc->startTrajectory(request->trajectory_id, request->timescale, request->reversed,
                                      request->group_mask);
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(broadcasts_delay_between_repeats_ms_));
        }
    ...
    void go_to(const std::shared_ptr<GoTo::Request> request, std::shared_ptr<GoTo::Response> response)
    {
        RCLCPP_INFO(logger_, "go_to(position=%f,%f,%f m, yaw=%f rad, duration=%f s, group_mask=%d)", request->goal.x,
    ...
    template <class T> void broadcast_set_param(const std::string &group, const std::string &name, const T &value)
    {
        for (int i = 0; i < broadcasts_num_repeats_; ++i)
        {
            for (auto &bc : broadcaster_)
    ...
    void update_name_to_id_map(const std::string &name, uint8_t id)
    {
        ...
        {
            RCLCPP_WARN(logger_, "At least two objects with the same id (%d, %s, %s)", id, name.c_str(),
                        iter->first.c_str());
        }
        else
        {
